...
export function parsePage(html, pageUrl) {
  ...
  // Validación AEO/GEO: pasamos los blocks raw al validador, luego no los exponemos
  const schemaValidation = validateSchemas(schema.blocks, { pageType });
  const { blocks: _rawBlocks, ...schemaPublic } = schema;
...
/**
 * Infiere tipo de página desde la URL (para sugerir schemas faltantes)
 */
function inferPageType(url) {
  if (!url) return 'unknown';
  const u = url.toLowerCase();
  if (/\/blog\/|\/article\/|\/post\/|\/news\//.test(u)) return 'blog';
  if (/\/product\/|\/shop\/|\/store\//.test(u)) return 'product';
  if (/\/faq|\/preguntas|\/help/.test(u)) return 'faq';
  if (/\/contact|\/contacto|\/store-locator/.test(u)) return 'local';
  if (/\/about|\/nosotros|\/sobre/.test(u)) return 'about';
  return 'home';
}
...
function extractSchema(html) {
  ...
  return {
    count: blocks.length,
    types: typesArr,
    blocks, // ← bloques raw para que el validador pueda procesarlos
